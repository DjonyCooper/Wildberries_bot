    ...
    if not check_user:
        ...
        if create['result']:
            ...
            await bot.send_message(message.chat.id,
                                   text=emoji.emojize(':handshake: Привет! Я создан, чтобы помочь тебе'
        ...
        if user_group == 'dev':
            ...
            await bot.send_message(message.chat.id,
                                   text=emoji.emojize(
                                       ':handshake: "Вы успешно инициализированы ботом!\nДоступ с повышенными правами!'),
...
async def get_text_messages(message):
    ...
    if message.text == emoji.emojize(':plus: Добавить'):
    ...
    elif message.text == emoji.emojize(':heavy_dollar_sign: Список моих хотелок :heavy_dollar_sign:'):
        ...
        if user_group == 'dev':
            ...
            markup.add(types.InlineKeyboardButton(f"Отмена", callback_data='no'))
            await bot.send_message(message.chat.id,
                                   text='Выберите, кому хотите отправить сообщение?',
                                   reply_markup=markup)
        ...
        if user_group == 'dev':
            await bot.send_message(message.chat.id,
                                   text=emoji.emojize(
                                       ':play_button: Переход в раздел: Главное меню\nДоступ с повышенными правами!'),
                                   reply_markup=start_keyboard.dev_keyboard())
        elif user_group == 'user':
            await bot.send_message(message.from_user.id,
...
# Обработка действий кнопки "Отправить сообщение" (ловим callback с кнопки выбора пользователя)
        ...
        try:
            ...
            if callback_query.data == 'no':
                await bot.send_message(callback_query.from_user.id,
                                       emoji.emojize(':speaking_head: процесс прерван пользователем.'))
                await state.finish()
            else:
                user_info['id'] = callback_query.data
                markup = types.InlineKeyboardMarkup()
                markup.add(types.InlineKeyboardButton(text='Отмена', callback_data="no"))
                await bot.send_message(callback_query.from_user.id,
                                       text=f"Напишите сообщение, которое хотите отправить пользователю с ID: {user_info['id']} или нажмите Отмена",
                                       reply_markup=markup)
                await state.finish()
                await SaveInfo.send_message_step_two.set()


# Обработка действий кнопки "Отправить сообщение" (ловим message - прерываем процесс)
...
async def send_message_step_one_msg(message: types.Message, state: FSMContext):
    async with state.proxy():
        try:
            ...
            await bot.send_message(message.chat.id,
                                   emoji.emojize(':speaking_head: процесс прерван, т.к. получены некорректные данные.'))
            await state.finish()


# Обработка действий кнопки "Отправить сообщение" (ловим message для пользователя - отправляем сообщение пользователю)
...
async def send_message_step_two_msg(message: types.Message, state: FSMContext):
    async with state.proxy() as user_info :
        try:
            ...
            msg = f'Сообщение от тех. поддержки бота:\n{message.text}'
            if telegram_id != '' and msg != '':
                await bot.send_message(telegram_id, text=msg)
                await bot.send_message(message.chat.id,
                                       emoji.emojize(f':speaking_head: Ваше сообщение успешно отправлено пользователю c ID: {telegram_id}.'))
            else:
                await bot.send_message(message.chat.id,
                                       emoji.emojize(f':speaking_head: Отправка сообщения пользователю c ID: {telegram_id} не выполнена, т.к. не были получены все необходимые для этого данные'))
            await state.finish()


# Обработка действий кнопки "Отправить сообщение" (ловим callback кнопки Отмена)
